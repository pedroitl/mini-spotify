...
private ArrayList<Midia> midias = new ArrayList();
	...
	public ArrayList<Midia> buscarMidia(Scanner scanner) throws ListaVaziaException, EntradaVaziaException, OpcaoInvalidaException {
	...
	public ArrayList<Midia> buscarTitulo(Scanner scanner) throws EntradaVaziaException {
        ArrayList<Midia> midiaEncontrada = new ArrayList<>();
        System.out.println("Digite o nome da mídia que você procura:");
        String titulo = scanner.nextLine();
        if (titulo.trim().isEmpty()) {
            throw new EntradaVaziaException("A entrada não pode ser vazia!");
        }
        for (Midia midia : midias) {
            if (midia.getTitulo().trim().equalsIgnoreCase(titulo)) {
                System.out.println(midia+"\n");
                midiaEncontrada.add(midia);
            }
        }
        if (midiaEncontrada.isEmpty()) {
            System.out.println("Não há mídias com esse título");
        }
        return midiaEncontrada;
    }

	private ArrayList<Midia> buscarGenero(Scanner scanner) throws EntradaVaziaException {
			ArrayList<Midia> midiaEncontrada = new ArrayList<>();
			...
			for (int indice = 0; indice<midias.size();indice++) {
				...
				case "ROCK":
					if(midias.get(indice).getGenero() == Genero.ROCK) {
						...
						midiaEncontrada.add(midias.get(indice));
					}else {
			...
			if (midiaEncontrada.isEmpty()) {
		        System.out.println("Nenhuma mídia cadastrada nesse gênero!");
		    }

		    return midiaEncontrada;
	}
	private ArrayList<Midia> buscarArtista(Scanner scanner) throws ListaVaziaException,EntradaVaziaException {
		ArrayList<Midia> midiaEncontrada = new ArrayList<>();	
	...
	private ArrayList<Midia> menuMidia(Scanner scanner) throws OpcaoInvalidaException, ListaVaziaException, EntradaVaziaException {
		ArrayList<Midia> midia = new ArrayList<>();
		try {
			int opcao =0;
			while(opcao<1||opcao>3) {
	...
	public void removerMidia(Scanner scanner) throws ListaVaziaException, EntradaVaziaException, OpcaoInvalidaException {
		try {
			ArrayList<Midia> midia = buscarMidia(scanner);
			if(!midia.isEmpty()) {
				for(Midia remover: midia) {
					midias.remove(remover);
